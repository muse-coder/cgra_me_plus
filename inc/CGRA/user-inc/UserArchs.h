...
class UserArchs
{
    public:
        UserArchs()
        {
            ...
            archs.push_back(std::make_pair(createMeshPlus, std::make_pair("Simple Mesh Plus CGRA Architecture", std::map<std::string, std::string>
                    {
                            {"cols", "4"}, // Columns
                            {"rows", "4"}, // Rows
                            {"homogeneous_fu", "1"}, // Homogeneous Function Unit ( 1 = true, 0 = false)
    ...
    private:
        ...
        static std::unique_ptr<CGRA> createMeshPlus(const std::map<std::string, std::string> & args);
